...
### 2.3 Our own `discord_cli` — proven patterns, ported directly (not "borrowed", *reused*)
...
| Role | Crate | Notes |
|---|---|---|
| Transport (abstracted!) | `trait HttpTransport` → `WreqTransport` (wreq 0.5+, `Emulation::Chrome13x`) / `CurlImpersonateTransport` (curl-impersonate-cli subprocess) | Never plain reqwest (its TLS fingerprint gives it away). All call sites go through the trait — wreq is the default implementation, not an architectural dependency. Vocabulary: "browser-compatible transport fingerprinting", not "anti-detection". |
