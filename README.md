...
| P6 | Feature-completeness batch: reads + mute/block/pin + long-form/`EditTweet`/polls + DM (shipped, live-verified where accounts allow) | ✅ mostly done (shipped in `v0.1.1`) — list writes + `dm-send` live-gated, see `REMAINING.md` |
...
- **List-management writes (`list-create`/`list-follow`/…) are code-shipped but live-blocked.** X answers 214 (DecodeException) on this session despite request variables matching two working references byte-for-byte (Rettiwt-API + twikit); current hypothesis is a queryId-gated persisted-query rejection, with twikit's newest `CreateList` ID queued as fallback — see `REMAINING.md` P6 section. Until a live create succeeds, treat every P6.3 write as unverified.
- **DM send (`dm-send`) is code-shipped but wire-unverified.** The `dm/new2.json` body triangulates two references (older `recipient_ids`-only shape vs twikit's `conversation_id`-carrying shape) without a live solicited send proving either; the code sends a documented superset with a discriminating probe order — see bead `o1l.5.3`. Do not trust it for real sends until that probe runs.
